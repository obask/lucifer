...
static int
ends(struct stemmer * z, const char * s)
{  int length = s[0];
   char * b = z->b;
   int k = z->k;
...
/* setto(z, s) sets (j+1),...k to the characters in the string s, readjusting k. */

static void
setto(struct stemmer * z, const char * s)
{
   int length = s[0];
   int j = z->j;
...
static void
r(struct stemmer * z, const char * s)
{
  if (m(z) > 0) {
    setto(z, s); 
  }
